...
@Component({
  ...
  template: `
    <div class="price__discount" [ngClass]="{ 'price__discount-hidden': !price?.discount }">
      <div class="price__prev">
        <span class="price__prev-line">{{price?.value | number | customCurrency}}</span> 
  ...
  styles: [`.price__discount{padding: 4px 0px;} .price__discount-bage{background-color: red;} .price__discount-hidden{ display: none;}
            .price{ font-weight: bold; padding: 4px 0px;} .price__prev-line{text-decoration: line-through;}`
  ]
})
